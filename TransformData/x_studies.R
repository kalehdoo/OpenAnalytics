...
in_path_studies <-
  paste(var_DIR_HOME,
        "Data/ACCT/DATA/unzipSrcFiles/studies.txt",
        sep = "")
...
studies_1 <- mutate(
  ...
  "flag_study_first_posted" = if_else(str_length(study_first_posted_date) >
                                        0, 1, 0),
  ...
  "flag_results_posted" = if_else(
    parse_date_time(
      as.factor(results_first_posted_date),
      orders = c("ymd", "dmy", "mdy")
    ) <= as.Date(var_current_date),
    1,
    0
  ),
  
  "actual_study_enrollment" = ifelse(enrollment_type == "Actual", enrollment, NA),
  "anticipated_study_enrollment" = ifelse(enrollment_type ==
                                            "Anticipated", enrollment, NA),
  "flag_has_dmc" = case_when(has_dmc == "t" ~ 1,
                             has_dmc == "f" ~ 0,
                             TRUE ~ NA_real_),
  "register_to_start_days" = difftime(
    parse_date_time(as.factor(start_date), orders = c("ymd", "dmy", "mdy")),
    parse_date_time(
      as.factor(study_first_posted_date),
      orders = c("ymd", "dmy", "mdy")
    ),
    units = "days"
  ),
  "start_to_complete_days" = difftime(
    parse_date_time(as.factor(completion_date), orders = c("ymd", "dmy", "mdy")),
    parse_date_time(as.factor(start_date), orders = c("ymd", "dmy", "mdy")),
    units = "days"
  ),
  "complete_to_results_days" = difftime(
    parse_date_time(
      as.factor(results_first_posted_date),
      orders = c("ymd", "dmy", "mdy")
    ),
...
write.table(
  studies_1,
  paste(var_DIR_HOME, "Data/ACCT/DATA/warehouse/x_studies.txt", sep = ""),
  sep = "|",
  row.names = FALSE
)
