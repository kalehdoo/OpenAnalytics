...
dest_logfile<-paste(var_DIR_ACCT_LOGS,var_filename_log, sep="")
...
destdir<-paste(var_DIR_ACCT_HOME, "DATA/SourceFiles/", sep="")
...
dest_unzip_dir<-paste(var_DIR_ACCT_HOME,"DATA/unzipSrcFiles", sep="")
dest_unzip_filepath<-paste(var_DIR_ACCT_HOME,"DATA/unzipSrcFiles", sep="")

unzip(destfile, exdir = dest_unzip_dir)

write(paste("UnzipComplete_ACCT",now(), sep="="), dest_logfile, append=TRUE)
write(paste("--END--"), dest_logfile, append=TRUE)
